...
func _ready() -> void:
	...
	# Init HP bar directly (player._ready fires before this)
	player_hp_bar.min_value = 0
	player_hp_bar.max_value = player.MAX_HP
	player_hp_bar.value     = player.MAX_HP

	# Connect enemy signals (deferred — ensures all _ready() calls complete first)
	call_deferred("_connect_all_enemies")
...
func _on_damage_taken(amount: int, world_pos: Vector2) -> void:
	pass  # damage numbers skipped in prototype
